...
class MethodNewInstanceIntercept {
  ...
  static void add_newInstanceReadOnly(ClassVisitor cv, ClassMeta meta) {
    ...
    mv.visitMethodInsn(INVOKESPECIAL, meta.getClassName(), INIT, "(L" + C_INTERCEPT_I + ";)V", false);
  ...
  static void add_initSuper(ClassVisitor cv, ClassMeta meta) {
    ...
    mv.visitVarInsn(ALOAD, 1);
    mv.visitMethodInsn(INVOKESPECIAL, meta.getSuperClassName(), "<init>", "(L" + C_INTERCEPT_I + ";)V", false);
    Label label1 = new Label();
    mv.visitLabel(label1);
